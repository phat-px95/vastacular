...
VAST.prototype.resolveWrappers = function resolveWrappers(/*maxRedirects, callback*/) {
    ...
    return nodeifyPromise(LiePromise.all(this.map('wrappers', function requestVAST(wrapper) {
        ...
        result.vast = result.vast ? Array.from(result.vast.push([vast.toPOJO()])) : [vast.toPOJO()];
        if (result.get('wrappers.length') > 0) {
            return result.resolveWrappers(maxRedirects - 1,null, timeStamps);
        }
...
VAST.fetch = function fetch(uri/*, options, callback*/) {
    ...
    return nodeifyPromise(fetchPromise.then(function makeVAST(response) {
            ...
            return options.resolveWrappers ?
                vast.resolveWrappers(options.maxRedirects, options.prohibitedUrls, timeStamps) : vast;
        }), callback);
};
