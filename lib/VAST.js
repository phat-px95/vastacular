...
VAST.prototype.resolveWrappers = function resolveWrappers(/*maxRedirects, callback, savedData, timestampofrequest*/) {
    var maxRedirects = isNaN(arguments[0]) ? Infinity : arguments[0];
    var callback = typeof arguments[0] === 'function' ? arguments[0] : arguments[1];
    var prohibitedUrls = Array.prototype.isPrototypeOf(arguments[1]) ?
        arguments[1] : [];
    var data = arguments[2];
    var timeStamps = arguments[arguments.length - 1];
    ...
    return nodeifyPromise(LiePromise.all(this.map('wrappers', function requestVAST(wrapper) {
        if (prohibitedUrls.indexOf(wrapper.vastAdTagURI) + 1) {
          return LiePromise.reject(new Error('Found prohibited URL'));
        }
        timeStamps.push({startedTime: new Date().getTime(), finishedTime: null});
        return LiePromise.resolve(
            request.get(wrapper.vastAdTagURI).withCredentials())
            .catch(function (error) {
                ...
                timeStamps[timeStamps.length - 1].finishedTime = new Date().getTime();
                if (response.errors) {
                    return response;
                }
                return {
        ...
        var errors = configs.filter(function(config) { return config.errors; });
        if (errors.length > 0) {
            return errors[0];
        }
        ...
        if (result.vast) {
            if (result.errors) {
                result.vast.push(result.errors);
                return result;
            } else {
                result.vast.push(vast.toPOJO());
            }
        } else {
            if (result.errors) {
                result.vast = [result.errors];
                return result;
            }
            result.vast = [vast.toPOJO()];
        }
        if (result.get('wrappers.length') > 0) {
            return result.resolveWrappers(maxRedirects - 1, null, result, timeStamps);
        }
        return result;
    }), callback);
};
...
VAST.fetch = function fetch(uri/*, options, callback*/) {
    ...
    var fetchPromise;
    var timeStamps = [];
    if (String(uri || '').match(/^data:/)) {
	    fetchPromise = LiePromise.resolve(request.get(uri).set(options.headers || {}));
    } else {
	    fetchPromise = LiePromise.resolve(request.get(uri).withCredentials().set(options.headers || {}));
    }
    timeStamps.push({startedTime: new Date().getTime(), finishedTime: null})
    return nodeifyPromise(fetchPromise.then(function makeVAST(response) {
        timeStamps[timeStamps.length - 1].finishedTime = new Date().getTime();
        try {
            var vast = new VAST(VAST.pojoFromXML(response.text));
            return options.resolveWrappers ?
                vast.resolveWrappers(options.maxRedirects, options.prohibitedUrls, vast, timeStamps) : vast;
        } catch (error) {
            timeStamps.push({startedTime: new Date().getTime(), finishedTime: null});
            const result = {vast: [], timeStamps: []};
            result.vast.push(error);
            result.timeStamps = timeStamps;
            return result;
        }
    }), callback);
